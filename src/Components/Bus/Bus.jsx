...
const endpoints = ["http://xmlopen.rejseplanen.dk/bin/rest.exe/departureBoard?id=851400602&rttime&format=json&useBus=1", "http://xmlopen.rejseplanen.dk/bin/rest.exe/departureBoard?id=851973402&rttime&format=json&useBus=1"]
...
export const Bus = () => {
    ...
    const handleExtractEndpoints = () => {
        ...
        endpoints.forEach(async (endpoint) => {
            try {
                ...
                setBusTimes(prevState => prevState.concat(extractedTimes?.DepartureBoard.Departure)); // Merge with prevState
    ...
    const sortExtractedTimes = useCallback(() => {
        ...
        let busIntervals = sortedTimes.filter(bus => bus.time.replace(":", "") >= time).splice(0, 5);

        // Add take off
        for (let i = 0; i < busIntervals.length; i++) {
            let num = busIntervals[i].time.replace(":", "");
            let timeDifference = String(Number(num) - Number(time));
            busIntervals[i] = { ...busIntervals[i], takeOff: timeDifference < 10 ? '0' + timeDifference : timeDifference };
        }
